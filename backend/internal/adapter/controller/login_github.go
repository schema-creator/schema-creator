...
func GitHubLogin(gitHubLogin *interactor.GitHubLogin,
	cookieSetter *cookie.CookieSetter) func(ctx echo.Context) error {
	return func(ctx echo.Context) error {
		...
		userInfo, err := gitHubLogin.GitHubLogin(ctx.Request().Context(), reqBody.AuthorizationCode, userAgent)
		if err != nil {
			log.Error(ctx.Request().Context(), "failed to github login", err)
			return echo.ErrInternalServerError
		}
